...
void insert(Stu** student, string id, string _name)  //无法修改主函数里的值
{
...
void delete_ID(s** _head,string id)    //无法改变主函数中的值
{
	int flag = 0;
	s* tmp;
	tmp = (*_head)->next;
...
void delete_name(s** _head,string _name)		//无法改变主函数中的值
{
	int flag = 0;
	s* tmp;
	tmp = (*_head)->next;
